...
export default function PoznajKolorystyke({ kolory, elementy }: PoznajKolorystykeProps) {
  ...
  const handle3DClick = () => {
    try {
      ...
      if (isAndroid) {
        // Zamiast naszej wiaty, ładujemy oficjalny model testowy od Google/Khronos
        const testModelUrl = 'https://raw.githubusercontent.com/KhronosGroup/glTF-Sample-Models/master/2.0/Avocado/glTF-Binary/Avocado.glb';
        
        // Używamy zdekodowanego URL
        const intentUrl = `intent://arvr.google.com/scene-viewer/1.0?file=${encodeURIComponent(testModelUrl)}&mode=3d_preferred&title=Test%20Awokado#Intent;scheme=https;package=com.google.android.googlequicksearchbox;action=android.intent.action.VIEW;S.browser_fallback_url=https://developers.google.com/ar;end;`;
