...
export default function AppleHeroEntrance({ videoUrl, imageUrl, title, subtitle, textColor, glowColor, layoutVariant = 'center-split', bottomGradientMode = 'dark' }: AppleHeroEntranceProps) {
  ...
  const isLight = bottomGradientMode === 'light';
  const bgClass = isLight ? 'bg-white' : 'bg-black';

  return (
    <div ref={containerRef} className={`relative w-full ${bgClass}`}>

      {/* ================================================================= */}
      {/* 📱 WERSJA MOBILE (Autoplay Hero - Isolated)                       */}
      {/* ================================================================= */}
      <div className={`block md:hidden relative ${isLight ? 'h-[50vh]' : 'h-[75vh]'} w-full overflow-hidden ${bgClass}`}>
        <div className="absolute inset-0 flex items-center justify-center z-0">
          <motion.div
            initial={isLight ? undefined : { y: -60 }}
        ...
        <div className={`relative z-30 h-full w-full flex flex-col items-center ${isLight ? 'pt-[8vh]' : 'pt-[15vh] -translate-y-[60px]'} px-6 pointer-events-none`}>
          <motion.div
          ...
          >
            <span className="text-[28px] font-semibold tracking-tight text-center inner-shine-text" style={{ animationDelay: '2.9s', color: activeTextColor, '--shine-color': activeTextColor } as React.CSSProperties}>
              {activeTitle.split(' ').slice(0, 2).join(' ')}
            </span>
            <h1 className={`text-[52px] font-semibold tracking-tight leading-none text-center font-sans ${isLight ? 'mt-[120px]' : 'mt-[250px]'} inner-shine-text`} style={{ animationDelay: '2.9s', color: activeTextColor, '--shine-color': activeTextColor } as React.CSSProperties}>
              {activeTitle.split(' ').slice(2).join(' ')}
            </h1>
          </motion.div>
        </div>

        <div className={`absolute bottom-0 left-0 w-full ${isLight ? 'h-[22vh]' : 'h-[40vh]'} pointer-events-none z-10 ${
          isLight
            ? 'bg-gradient-to-t from-white via-white via-white/80 via-white/20 to-transparent'
            ...
            {bottomGradientMode !== 'none' && (
              <div className={`absolute bottom-0 left-0 w-full h-[25vh] pointer-events-none z-10 ${bottomGradientMode === 'light' ? '' : 'backdrop-blur-[4px]'}`}>
                ...
                <div className="absolute inset-x-0 bottom-0 h-full" style={{
                  background: bottomGradientMode === 'light'
